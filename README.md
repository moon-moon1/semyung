...
**DialoGPT**란?
	Reddit의 댓글과 토론 스레드에서 1억 4700만 개의 대화를 추출하여 학습된 GPT 모델입니다. DialoGPT 아키텍처는 GPT-2를 기반으로 한 자기 회귀 언어 모델로 설계된 다층 변환기를 모델 아키텍처로 사용합니다. GPT-2 모델과 DialoGPT와 다른 점은 GPT-2 모델은 일반 텍스트를 기반으로 두지만 DialoGPT는 토론 스레드를 기반으로 학습합니다.

GPT-2의 아키텍처는 아래와 같이 두 가지 유형의 블록으로 존재합니다.
![GPT-2 아키텍처](file:///Users/jeong-yongseog/Desktop/DialoGPT/%E1%84%80%E1%85%A8%E1%84%8E%E1%85%B3%E1%86%BC.avif)

- 인코더 - Self-Attention layer과 feed-forward neural network를 포함합니다.
- 디코더 - 인코더와 유사하지만 Self-Attention layer이 마스킹 됩니다.

Self-Attention layer는 현재 단어(텍스트의 연속된 단어)의 오른쪽에 있는 토큰을 정점으로 삼는 위치를 허용하는 반면, 마스크된 자기 주의 계층은 그런 일이 발생하지 않도록 방지합니다. 

GPT-2는 트랜스포머 디코더 블록을 사용하여 구축되었습니다. 즉, 아키텍처에는 다음과 같은 계층이 사용됩니다.

1. 임베딩 계층(Embedding Layer) – 입력 텍스트를 임베딩으로 변환하는 역할을 담당합니다(각 단어는 고정 길이 벡터 표현으로 변환됨)
2. 트랜스포머 디코더(Transformer Decoder) - 마스킹된 자기 주의 및 피드 포워드 신경망 계층을 갖춘 여러 디코더 블록 포함
3. 출력 계층(Output Layer) – 디코더에서 얻은 임베딩을 단어로 변환하는 역할을 담당합니다.

- **자기 회귀 모델**이란?
자기 회귀 모델은 시퀀스의 이전 입력에서 측정값을 가져와 시퀀스의 다음 성분을 자동으로 예측하는[기계 학습(ML)](https://aws.amazon.com/what-is/machine-learning/)모델의 클래스입니다. 자기 회귀는 시계열 분석에 사용되는 통계 기법으로, 시계열의 현재 값이 과거 값의 함수라고 가정합니다. 자기 회귀 모델은 유사한 수학적 기법을 사용하여 시퀀스에 있는 요소 간의 확률적 상관관계를 판단합니다. 그런 다음 도출된 정보를 사용하여 알려지지 않은 시퀀스의 다음 요소를 추측합니다. 예를 들어 자기 회귀 모델이 훈련 중에 여러 영어 문장을 처리하여 단어 ‘is’가 항상 ‘there’라는 단어 뒤에 오는 것을 파악합니다. 그런 다음 ‘thereis’가 함께 있는 새 시퀀스를 생성합니다. 
